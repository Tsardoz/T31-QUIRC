...
#include <opencv2/highgui.hpp>   // For window management
...
extern "C" {
	//#include "utils.h" // Ensure this file has the corrected QROutput struct
...
int main(int argc, char* argv[]) {
    ...
    // Start timing CLAHE processing
	auto start = std::chrono::high_resolution_clock::now();

	// Apply CLAHE to the resized image
	//int res = CLAHE(grayscaleImagedata, 640, 360, 0, 255, 10, 10, 256, 6.0);
	//if (res != 1) {
	//	std::cerr << "CLAHE processing failed.\n";
	//}
	struct QROutput *outputs = scan_qr_with_quirc(grayscaleImage.data, grayscaleImage.cols, grayscaleImage.rows, &outputCount);

    // Stop timing CLAHE processing
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);

    std::cout << "CLAHE processing time: " << duration.count() << " milliseconds\n";
	for (unsigned int i = 0; i < outputCount; ++i) {
    	if (outputs[i].data != NULL) {
        	std::cout << "Decoded QR message: " << outputs[i].data << std::endl;
    	} else {
        	std::cout << "QR code decoding failed for index " << i << std::endl;
    	}
	}
    //if (!cv::imwrite(argv[2], claheImage)) {
	if (!cv::imwrite(argv[2], grayscaleImage)) {
        std::cerr << "Failed to write the image: " << argv[2] << std::endl;
        return -1;
    }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& entry : fs::directory_iterator(folderPath)) {
        if ((entry.path().extension() == ".png") or (entry.path().extension() == ".jpg")) { // Ensure it's a JPEG image
    ...
    if (filePaths.empty()) {
        std::cerr << "No JPEG images found in the folder\n";
        return -1;
    }

    // Load the first image to determine the size for VideoWriter
    cv::Mat firstFrame = cv::imread(filePaths.front(), cv::IMREAD_COLOR);
    if (firstFrame.empty()) {
        std::cerr << "Could not read the first image\n";
        return -1;
    }

    // Initialize VideoWriter
    cv::VideoWriter videoWriter("output_video.mp4", cv::VideoWriter::fourcc('M','P','4','V'), 24, firstFrame.size(), true);
    ...
    for (const auto& filePath : filePaths) {
        ...
        if (frame.empty()) {
            std::cerr << "Could not read the image: " << filePath << std::endl;
            continue;
        }
        ...
        int outputCount;
        QROutput* outputs = scan_qr_with_quirc(grayFrame.data, grayFrame.cols, grayFrame.rows, &outputCount);

        for (int i = 0; i < outputCount; ++i) {
            if (outputs[i].data != NULL) {
                std::vector<cv::Point> points;
                for (int j = 0; j < 4; ++j) {
                    points.push_back(cv::Point(outputs[i].corners[j].x, outputs[i].corners[j].y));
                }
